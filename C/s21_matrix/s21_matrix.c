...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  int flag = (check_correctness_of_matrix(A) ||
              check_correctness_of_matrix(B) || result == NULL) > 0
                 ? 1
                 : (check_equivalence_of_sizes(A, B) ? COMPUTATION_FAIL : OK);
  if (!flag) {
...
int s21_mult_number(matrix_t *A, double number, matrix_t *result) {
  int flag = (check_correctness_of_matrix(A) || result == NULL) > 0 ? INCORRECT_MATRIX : OK;
  if (!flag) {
    flag = s21_create_matrix(A->rows, A->columns, result);
    if (!flag)
      for (int i = 0; i < result->rows; ++i)
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  int flag = (check_correctness_of_matrix(A) ||
              check_correctness_of_matrix(B) || result == NULL) > 0
                 ? 1
                 : (A->columns != B->rows ? COMPUTATION_FAIL : OK);
  if (!flag) {
...
int s21_determinant(matrix_t *A, double *result) {
  int flag = (check_correctness_of_matrix(A) || result == NULL) > 0
                 ? 1
                 : (A->columns != A->rows ? COMPUTATION_FAIL : OK);
  if (!flag) {
    if (A->columns == 1)
      *result = A->matrix[0][0];
    else if (A->columns == 2)
      ...
      for (int i = 0; i < result->rows; ++i)
        for (int j = 0; j < result->columns; ++j) {
          ...
          if(!flag) {
            ...
            int det_flag = s21_determinant(&auxil_matrix, &auxil_det);
            if (!det_flag) 
              result->matrix[i][j] = auxil_det * minus_one(i, j);
            s21_remove_matrix(&auxil_matrix);
            flag = det_flag;
          }
        }
...
int s21_inverse_matrix(matrix_t *A, matrix_t *result) {
  double det = 1.0;
  int flag = (result == NULL) > 0 ? INCORRECT_MATRIX : s21_determinant(A, &det);
  flag = flag > 0 ? flag : (det < 1e-7) * 2;
  if (!flag && A->rows > 1) {
    matrix_t auxil_matrix = {NULL};
